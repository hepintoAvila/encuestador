...
function ejecutarConsulta($nusuario, $email, $conexion) {
    try {
        ...
        $consulta = "SELECT COUNT(*) AS num FROM tab_usuarios WHERE idUsuario = :idUsuario";
...
/*Al incluir el archivo "base_de_datos.php", todas sus variables están
						 ...
						 */
							...
							$resultado = ejecutarConsulta($idUsuario,$conexion);
							// Hacer algo con el resultado
							if ($resultado==0) {
								$sentencia = $conexion->prepare("SELECT fun_insert_asistentes(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);");
								$resultado = $sentencia->execute([$id, $idUsuario, $apellido1, $apellido2, $nombre1,$nombre2,$genero,$fec_nac,$rh,$telefono,$email]); 
								$row[] = array(
								'id'=>1,
